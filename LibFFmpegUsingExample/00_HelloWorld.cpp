...
static int decode_packet(AVPacket* pPacket, AVCodecContext* pCodecContext, AVFrame* pFrame)
{
    ...
    while (response >= 0)
    {
        ...
        if (response >= 0)
        {
            ...
            snprintf(
                frame_filename, sizeof(frame_filename), "%s-%d.pgm", "frame", pCodecContext->frame_number);
            ...
            {
                logging("Warning: the generated file may not be a grayscale image, but could e.g. be just "
                        "the R component if the video format is RGB");
            }
            // save a grayscale frame into a .pgm file
            save_gray_frame(
                pFrame->data[0], pFrame->linesize[0], pFrame->width, pFrame->height, frame_filename);
        }
    }
    return 0;
}
